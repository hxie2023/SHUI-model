...
path<-"/home/smq/code/1/tyh_20240406/res1014.txt"
setwd(cwd)
...
cal7 <-subset(d7 ,select = c("date","TC.x","TC.y"))#X:OBS;Y:SIM
cal5 <-subset(d5 ,select = c("date","TC.x","TC.y"))
cal17<-subset(d17,select = c("date","TC.x","TC.y"))
cal15<-subset(d15,select = c("date","TC.x","TC.y"))
cal20<-subset(d20,select = c("date","TC.x","TC.y"))
cal23<-subset(d23,select = c("date","TC.x","TC.y"))
...
stopifnot(length(cal7 $TC.x) == length(cal7 $TC.y))
stopifnot(length(cal5 $TC.x) == length(cal5 $TC.y))
stopifnot(length(cal17$TC.x) == length(cal17$TC.y))
stopifnot(length(cal15$TC.x) == length(cal15$TC.y))
stopifnot(length(cal20$TC.x) == length(cal20$TC.y))
stopifnot(length(cal23$TC.x) == length(cal23$TC.y))
NSE_ctc7     <- cal_NSE(cal7 $TC.x, cal7 $TC.y)
NSE_ctc5     <- cal_NSE(cal5 $TC.x, cal5 $TC.y)
NSE_ctc17    <- cal_NSE(cal17$TC.x, cal17$TC.y)
NSE_ctc15    <- cal_NSE(cal15$TC.x, cal15$TC.y)
NSE_ctc20    <- cal_NSE(cal20$TC.x, cal20$TC.y)
NSE_ctc23    <- cal_NSE(cal23$TC.x, cal23$TC.y)

PBAIS_ctc7   <- cal_PBIAS(cal7 $TC.x, cal7 $TC.y)
PBAIS_ctc5   <- cal_PBIAS(cal5 $TC.x, cal5 $TC.y)
PBAIS_ctc17  <- cal_PBIAS(cal17$TC.x, cal17$TC.y)
PBAIS_ctc15  <- cal_PBIAS(cal15$TC.x, cal15$TC.y)
PBAIS_ctc20  <- cal_PBIAS(cal20$TC.x, cal20$TC.y)
PBAIS_ctc23  <- cal_PBIAS(cal23$TC.x, cal23$TC.y)

RMSE_ctc7    <-rmse(cal7 $TC.x, cal7 $TC.y)
RMSE_ctc5    <-rmse(cal5 $TC.x, cal5 $TC.y)
RMSE_ctc17   <-rmse(cal17$TC.x, cal17$TC.y)
RMSE_ctc15   <-rmse(cal15$TC.x, cal15$TC.y)
RMSE_ctc20   <-rmse(cal20$TC.x, cal20$TC.y)
RMSE_ctc23   <-rmse(cal23$TC.x, cal23$TC.y)

Rsq_ctc7     <-cor(cal7 $TC.x, cal7 $TC.y)^2
Rsq_ctc5     <-cor(cal5 $TC.x, cal5 $TC.y)^2
Rsq_ctc17    <-cor(cal17$TC.x, cal17$TC.y)^2
Rsq_ctc15    <-cor(cal15$TC.x, cal15$TC.y)^2
Rsq_ctc20    <-cor(cal20$TC.x, cal20$TC.y)^2
Rsq_ctc23    <-cor(cal23$TC.x, cal23$TC.y)^2

total  <- rbind(cal5,cal7,cal15,cal17,cal20,cal23)
t_r2   <- cor(total$TC.x, total$TC.y)^2
t_pbais<- cal_PBIAS( total$TC.x, total$TC.y)
...
fig7 <-subset(D7 ,select = c("date","TC.x","TC.y"))#X:OBS;Y:SIM
fig5 <-subset(D5 ,select = c("date","TC.x","TC.y"))
fig17<-subset(D17,select = c("date","TC.x","TC.y"))
fig15<-subset(D15,select = c("date","TC.x","TC.y"))
fig20<-subset(D20,select = c("date","TC.x","TC.y"))
fig23<-subset(D23,select = c("date","TC.x","TC.y"))
...
# # View(qq)
# qq$logObserved <- log10(qq$TC.x)
# qq$logSimulated <- log10(qq$TC.y)
...
#   labs(x = "TC Simulated (mg/L)", y = "Observed (mg/L)") +
#   theme_bw() #+
#   # theme(text = element_text(size = 16,family = "serif"),  # Set font to Times New Roman
#   #       axis.text = element_text(colour = "black",size = 16,family = "serif"),
#   #       axis.ticks = element_line(colour = "black", linewidth = 0.4),  # Customize ticks
#   #       axis.ticks.length = unit(-0.2, "cm"))  # Make ticks point inward 
...
fTC<- cowplot::plot_grid(p1,p2,p3,p4,p5,p6, nrow = 2)
fTC
