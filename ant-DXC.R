...
# data1<-read.table(file.path(cwd, "out/test4.txt"),header = T)
# prec <-read.table(file.path(cwd, "dat/precipitation.txt"),header = T)
# prec <-prec[,c(1,6)]
...
# testd7<-merge(D15,merge(data1,prec,by="date"),by="date",all=TRUE)
# View(testd7)

cal7 <-subset(d7 ,select = c("date","DXC.x","DXC.y"))#X:OBS;Y:SIM
cal5 <-subset(d5 ,select = c("date","DXC.x","DXC.y"))
cal17<-subset(d17,select = c("date","DXC.x","DXC.y"))
cal15<-subset(d15,select = c("date","DXC.x","DXC.y"))
cal20<-subset(d20,select = c("date","DXC.x","DXC.y"))
cal23<-subset(d23,select = c("date","DXC.x","DXC.y"))
...
# View(cal20)
# View(cal23)
stopifnot(length(cal7 $DXC.x) == length(cal7 $DXC.y))
stopifnot(length(cal5 $DXC.x) == length(cal5 $DXC.y))
stopifnot(length(cal17$DXC.x) == length(cal17$DXC.y))
stopifnot(length(cal15$DXC.x) == length(cal15$DXC.y))
stopifnot(length(cal20$DXC.x) == length(cal20$DXC.y))
stopifnot(length(cal23$DXC.x) == length(cal23$DXC.y))
NSE_ctc7     <- cal_NSE(cal7 $DXC.x, cal7 $DXC.y)
NSE_ctc5     <- cal_NSE(cal5 $DXC.x, cal5 $DXC.y)
NSE_ctc17    <- cal_NSE(cal17$DXC.x, cal17$DXC.y)
NSE_ctc15    <- cal_NSE(cal15$DXC.x, cal15$DXC.y)
NSE_ctc20    <- cal_NSE(cal20$DXC.x, cal20$DXC.y)
NSE_ctc23    <- cal_NSE(cal23$DXC.x, cal23$DXC.y)
PBAIS_ctc7   <- cal_PBIAS(cal7 $DXC.x, cal7 $DXC.y)
PBAIS_ctc5   <- cal_PBIAS(cal5 $DXC.x, cal5 $DXC.y)
PBAIS_ctc17  <- cal_PBIAS(cal17$DXC.x, cal17$DXC.y)
PBAIS_ctc15  <- cal_PBIAS(cal15$DXC.x, cal15$DXC.y)
PBAIS_ctc20  <- cal_PBIAS(cal20$DXC.x, cal20$DXC.y)
PBAIS_ctc23  <- cal_PBIAS(cal23$DXC.x, cal23$DXC.y)

RMSE_ctc7    <-rmse(cal7 $DXC.x, cal7 $DXC.y)
RMSE_ctc5    <-rmse(cal5 $DXC.x, cal5 $DXC.y)
RMSE_ctc17   <-rmse(cal17$DXC.x, cal17$DXC.y)
RMSE_ctc15   <-rmse(cal15$DXC.x, cal15$DXC.y)
RMSE_ctc20   <-rmse(cal20$DXC.x, cal20$DXC.y)
RMSE_ctc23   <-rmse(cal23$DXC.x, cal23$DXC.y)
Rsq_ctc7     <-cor(cal7 $DXC.x, cal7 $DXC.y)^2
Rsq_ctc5     <-cor(cal5 $DXC.x, cal5 $DXC.y)^2
Rsq_ctc17    <-cor(cal17$DXC.x, cal17$DXC.y)^2
Rsq_ctc15    <-cor(cal15$DXC.x, cal15$DXC.y)^2
Rsq_ctc20    <-cor(cal20$DXC.x, cal20$DXC.y)^2
Rsq_ctc23    <-cor(cal23$DXC.x, cal23$DXC.y)^2
# View(cal7)

total  <- rbind(cal5,cal7,cal15,cal17,cal20,cal23)
t_r2   <- cor(total$DXC.x, total$DXC.y)^2
t_pbais<- cal_PBIAS( total$DXC.x, total$DXC.y)
...
fig7 <-subset(D7 ,select = c("date","DXC.x","DXC.y"))#X:OBS;Y:SIM
fig5 <-subset(D5 ,select = c("date","DXC.x","DXC.y"))
fig17<-subset(D17,select = c("date","DXC.x","DXC.y"))
fig15<-subset(D15,select = c("date","DXC.x","DXC.y"))
fig20<-subset(D20,select = c("date","DXC.x","DXC.y"))
fig23<-subset(D23,select = c("date","DXC.x","DXC.y"))
...
# View(qq)
# qq$logObserved <- log10(qq$DXC.x)
# qq$logSimulated <- log10(qq$DXC.y)
...
#   labs(x = "DXC Simulated (mg/L)", y = "Observed (mg/L)") +
#   theme_bw() #+
# 
...
fDXC<- cowplot::plot_grid(p1, p2,p3, p4,p5, p6, nrow = 2)
fDXC
