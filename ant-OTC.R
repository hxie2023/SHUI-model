...
cal7 <-subset(d7 ,select = c("date","OTC.x","OTC.y"))#X:OBS;Y:SIM
cal5 <-subset(d5 ,select = c("date","OTC.x","OTC.y"))
cal17<-subset(d17,select = c("date","OTC.x","OTC.y"))
cal15<-subset(d15,select = c("date","OTC.x","OTC.y"))
cal20<-subset(d20,select = c("date","OTC.x","OTC.y"))
cal23<-subset(d23,select = c("date","OTC.x","OTC.y"))
...
stopifnot(length(cal7 $OTC.x) == length(cal7 $OTC.y))
stopifnot(length(cal5 $OTC.x) == length(cal5 $OTC.y))
stopifnot(length(cal17$OTC.x) == length(cal17$OTC.y))
stopifnot(length(cal15$OTC.x) == length(cal15$OTC.y))
stopifnot(length(cal20$OTC.x) == length(cal20$OTC.y))
stopifnot(length(cal23$OTC.x) == length(cal23$OTC.y))
NSE_ctc7     <- cal_NSE(cal7$OTC.x, cal7$OTC.y)
NSE_ctc5     <- cal_NSE(cal5$OTC.x, cal5$OTC.y)
NSE_ctc17    <- cal_NSE(cal17$OTC.x, cal17$OTC.y)
NSE_ctc15    <- cal_NSE(cal15$OTC.x, cal15$OTC.y)
NSE_ctc20    <- cal_NSE(cal20$OTC.x, cal20$OTC.y)
NSE_ctc23    <- cal_NSE(cal23$OTC.x, cal23$OTC.y)
PBAIS_ctc7   <- cal_PBIAS(cal7$OTC.x, cal7$OTC.y)
PBAIS_ctc5   <- cal_PBIAS(cal5$OTC.x, cal5$OTC.y)
PBAIS_ctc17  <- cal_PBIAS(cal17$OTC.x, cal17$OTC.y)
PBAIS_ctc15  <- cal_PBIAS(cal15$OTC.x, cal15$OTC.y)
PBAIS_ctc20  <- cal_PBIAS(cal20$OTC.x, cal20$OTC.y)
PBAIS_ctc23  <- cal_PBIAS(cal23$OTC.x, cal23$OTC.y)

RMSE_ctc7    <-rmse(cal7$OTC.x, cal7$OTC.y)
RMSE_ctc5    <-rmse(cal5$OTC.x, cal5$OTC.y)
RMSE_ctc17   <-rmse(cal17$OTC.x, cal17$OTC.y)
RMSE_ctc15   <-rmse(cal15$OTC.x, cal15$OTC.y)
RMSE_ctc20   <-rmse(cal20$OTC.x, cal20$OTC.y)
RMSE_ctc23   <-rmse(cal23$OTC.x, cal23$OTC.y)
Rsq_ctc7     <-cor(cal7$OTC.x, cal7$OTC.y)^2
Rsq_ctc5     <-cor(cal5$OTC.x, cal5$OTC.y)^2
Rsq_ctc17    <-cor(cal17$OTC.x, cal17$OTC.y)^2
Rsq_ctc15    <-cor(cal15$OTC.x, cal15$OTC.y)^2
Rsq_ctc20    <-cor(cal20$OTC.x, cal20$OTC.y)^2
Rsq_ctc23    <-cor(cal23$OTC.x, cal23$OTC.y)^2
...
t_r2   <- cor(total$OTC.x, total$OTC.y)^2
t_pbais<- cal_PBIAS( total$OTC.x, total$OTC.y)
res<- rbind(t_r2,t_pbais)
write.table(res,path,append =T,row.names=F,col.names=F,quote=F)


fig7 <-subset(D7 ,select = c("date","OTC.x","OTC.y"))#X:OBS;Y:SIM
fig5 <-subset(D5 ,select = c("date","OTC.x","OTC.y"))
fig17<-subset(D17,select = c("date","OTC.x","OTC.y"))
fig15<-subset(D15,select = c("date","OTC.x","OTC.y"))
fig20<-subset(D20,select = c("date","OTC.x","OTC.y"))
fig23<-subset(D23,select = c("date","OTC.x","OTC.y"))
...
# # View(qq)
# qq$logObserved <- log10(qq$OTC.x)
# qq$logSimulated <- log10(qq$OTC.y)
...
#   labs(x = "OTC Simulated (mg/L)", y = "Observed (mg/L)") +
#   theme_bw() #+
...
fOTC<- cowplot::plot_grid(p1, p2,p3, p4,p5, p6, nrow = 2)
fOTC